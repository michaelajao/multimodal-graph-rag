...
from generate_questions import (
    bad_reference, has_orphan_reference, leaks_modality, MAX_Q_CHARS,
)
...
for path in sys.argv[1:]:
    ...
    for item in items:
        q = item["q"]
        reason = ("container/source-id" if bad_reference(q) else
                  "over-length"        if len(q) > MAX_Q_CHARS else
                  "orphan reference"   if has_orphan_reference(q) else
                  "modality leak"      if leaks_modality(q) else None)
