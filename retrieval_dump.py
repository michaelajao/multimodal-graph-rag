...
from rag_basics import build_index, retrieve, CORPUS_DIR
...
def main(qfile):
    ...
    for item in questions:
        ...
        s1_hits += h1; s2_hits += h2; both_hits += (h1 and h2)
        out.append({"q": item["q"], "source": item["source"],
                    "source2": item.get("source2"), "top_k": top,
                    "source_in_topk": h1, "source2_in_topk": h2,
    ...
    print(f"\n{n} questions -> {dump}")
    print(f"source  in top-{K}: {s1_hits}/{n}  ({s1_hits/n:.3f})   <- matches summary Recall@3")
    print(f"source2 in top-{K}: {s2_hits}/{n}  ({s2_hits/n:.3f})")
    print(f"BOTH    in top-{K}: {both_hits}/{n}  ({both_hits/n:.3f})   <- completeness ceiling")
    print(f"\nQuestions missing source2 (the +KG-retrieval arm's rescue targets): "
          f"{n - s2_hits}")

if __name__ == "__main__":
    main(sys.argv[1] if len(sys.argv) > 1 else "questions_spiqa_multihop_cross.json")
