...
from rag_basics import chunk_text, load_file
from graph_aware import build_triples
...
OUT_FILE       = "questions_spiqa_multihop_cross.json"
SEED           = 42
MIN_ENTITY_LEN = 5          # MUST equal graph_facts_for_query's min_len
OVERGEN_FACTOR = 2.0        # generate ~2x, then skim manually

BANNED_WORDS = {"figure", "table", "chart", "image", "graph", "diagram",
                "plot", "caption", "photo", "picture"}
...
STRUCTURAL_WORDS = {"context", "contexts", "excerpt", "excerpts", "passage",
                    "passages", "paper", "papers", "source", "sources",
                    "first", "second", "former", "latter", "respectively",
                    "aforementioned"}
...
STOPLIST = {"model", "models", "method", "methods", "approach", "dataset",
            "datasets", "results", "training", "network", "networks",
            "algorithm", "performance", "accuracy", "learning", "system",
            "table", "figure", "section", "paper", "authors", "baseline",
            "experiments", "function", "features", "value", "values"}


def whole_word(entity: str, text: str) -> bool:
    return re.search(r"\b" + re.escape(entity.lower()) + r"\b", text.lower()) is not None
...
def collect_candidates(corpus_dir):
    ...
    print(f"Entities in the graph: {len(occurrences)}; "
          f"in >= 2 papers: {len(multi)}")
    return multi
...
def generate_pair_question(entity, src_a, chunk_a, src_b, chunk_b):
    prompt = PROMPT.format(entity=entity, src_a=src_a, src_b=src_b,
                           chunk_a=chunk_a[:1500], chunk_b=chunk_b[:1500])
...
def validate(qa, entity):
    ...
    from generate_questions import leaks_modality, has_orphan_reference
    q = qa["q"]
    if leaks_modality(q):
...
def main(n_target, verbose=False):
    ...
    print(f"Manually skim down to the best {n_target}: remove duplicates in spirit, "
