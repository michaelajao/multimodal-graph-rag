...
De-duplication uses the full configuration hash written by compare_all.py.
Legacy rows without a hash remain isolated by run_id.
...
def dedupe(rows):
    """Keep one row per system and complete configuration identity."""
    newest = {}
    for r in rows:
        identity = r.get("config_hash") or f"legacy:{r.get('run_id', '')}"
        k = (identity, r["system"])
    ...
    kept = [
        ...
        if r["run_id"]
        == newest[
            ((r.get("config_hash") or f"legacy:{r.get('run_id', '')}"), r["system"])
        ]
    ]
...
def main():
    ...
    ap.add_argument(
        "--keep-all", action="store_true", help="do not de-duplicate superseded runs"
    )
    args = ap.parse_args()
    ...
    rows.sort(
        key=lambda r: (
            order_key(r["question_set"], QSET_ORDER),
    ...
    write(
        os.path.join(RESULTS, "table_quality.csv"),
        ["question_type", "question_set", "model", "vendor", "access", "system"]
        + QUALITY_COLS,
        rows,
    )

    write(
        os.path.join(RESULTS, "table_efficiency.csv"),
    ...
    if figs:
        write(
            os.path.join(RESULTS, "table_figures.csv"),
            ["model", "vendor", "access", "system"]
            + QUALITY_COLS
            + ["mean_in_tok", "cost_per_100q_usd"],
            figs,
        )
            ...
            deltas.append(
                {
                    ...
                    "in_tok_delta": (
                        round(targ_tok - base_tok, 1)
                        if None not in (base_tok, targ_tok)
                        else ""
                    ),
                }
            )
    if deltas:
        deltas.sort(
            key=lambda d: (
                order_key(d["question_set"], QSET_ORDER),
                order_key(d["model"], MODEL_ORDER),
                d["contrast"],
            )
        )
        write(
            os.path.join(RESULTS, "table_kg_delta.csv"),
            [
                "question_type",
                "question_set",
                "model",
    ...
    def grid(metric, fmt, label):
        ...
        for qset in QSET_ORDER:
            models = sorted(
                {r["model"] for r in rows if r["question_set"] == qset},
            ...
            for model in models:
                ...
                for s in SYS_ORDER:
                    ...
                    cells += format(v, fmt).rjust(14) if v is not None else "".rjust(14)
    ...
    missing = [
        ...
        if not any(r["question_set"] == q and r["model"] == m for r in rows)
    ]
    if missing:
        print("\n" + "=" * 76)
