...
def has_orphan_reference(question: str) -> bool:
    ...
    body = re.sub(
        r"^\s*(what|how|which|when|where|who|why)\b", "", question, flags=re.I
    )
...
def gen_text_questions(
    client, page_text: str, page_id: str, multihop: bool = False
) -> list:
...
# ── Generation loops ──────────────────────────────────────────────────────────
def build_text_type(
    kind: str, target: int, corpus_dir: str, out_path: str, smoke: bool
) -> None:
    ...
    for page_id, text in pages:
        ...
        if attempts > target * 6:  # pages are noisy; don't loop forever
            print(
                f"  {WARN} stopping: {n_rejected} rejections in {attempts} "
                f"attempts — prompt may need loosening"
            )
            break

        new = gen_text_questions(client, text, page_id, multihop=(kind == "multihop"))
        if not new:
            n_rejected += 1
            continue
        ...
        if smoke:
            ...
            print(
                f"  [{len(items)}/{target}]  {DIM}rejected {n_rejected} "
                f"({rate:.0%}){RESET}"
            )
    ...
    if n_rejected:
        print(
            f"      {DIM}{n_rejected} rejected (vague / leaky / "
            f"orphan reference){RESET}"
        )


def build_figure_type(target: int, image_dir: str, out_path: str, smoke: bool) -> None:
    ...
    for crop_path in crops:
        ...
        if attempts > target * 6:
            print(
                f"  {WARN} stopping: {n_rejected + n_unverified} rejections "
        ...
        if not verify_figure_question(client, crop_path, new[0]):
            ...
            if smoke:
                print(
                    f"  {WARN} {os.path.basename(crop_path)} "
    ...
    if n_rejected or n_unverified:
        print(
            f"      {DIM}rejected {n_rejected} (vague/leaky), "
...
def main() -> int:
    ...
    ap.add_argument(
        "--prefix",
        default=os.path.join("data", "questions", "questions_publaynet"),
        help="output filename prefix",
    )
    ap.add_argument(
        "--only", choices=["text", "multihop", "figure"], help="generate one type only"
    )
    ap.add_argument(
        "--smoke",
        action="store_true",
    ...
    try:
        if args.only in (None, "text"):
            build_text_type(
                "text", targets["text"], args.corpus_dir, paths["text"], args.smoke
            )
        ...
        if args.only in (None, "figure"):
            build_figure_type(
                targets["figure"], args.image_dir, paths["figure"], args.smoke
            )
