...
# ── Retry wrapper ─────────────────────────────────────────────────────────────
def caption_image(crop: Image.Image, smoke: bool = False, max_retries: int = 5) -> str:
    ...
    for attempt in range(max_retries):
        try:
                ...
                messages=[
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": "Describe this figure or table from a scientific paper in one "
            ...
            print(
                f"  [caption] attempt {attempt + 1} failed ({e}); retrying in {wait}s …"
            )
            time.sleep(wait)
...
def ingest(limit: int, shards: int | None = None, smoke: bool = False) -> None:
    ...
    if already_done:
        print(
            f"Resuming: {already_done} pages already processed, "
    ...
    for row in load_stream(limit, shards=shards):
        ...
        if page_text:
            with open(
                os.path.join(CORPUS_DIR, f"{key}.txt"), "w", encoding="utf-8"
            ) as f:
                f.write("\n".join(page_text))
    ...
    if n_pages < limit and not smoke:
        ...
        print(f"    small-publaynet-wds is a subset and may not contain {limit} pages.")
        print(
            f"    Options: proceed with {n_pages} (fine if close to target), or switch"
        )
        print("    to the full PubLayNet source for more shards.")
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--limit", type=int, default=1000, help="total pages to ingest (default: 1000)"
    )
    ap.add_argument(
        "--shards",
        type=int,
