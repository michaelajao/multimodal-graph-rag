...
def safe_filename(title: str) -> str:
    ...
    ascii_t = unicodedata.normalize("NFKD", t).encode("ascii", "ignore").decode("ascii")
...
def main() -> int:
    ...
    print(f"{'=' * 74}")
    print(
        f"{DIM}  pool {pool_n} questions -> corpus | evaluate on {eval_n} "
    ...
    if os.path.exists(CORPUS_DIR) and glob.glob(os.path.join(CORPUS_DIR, "*.txt")):
        ...
        print(
            "      Re-run with --clean for a fresh corpus, or these will be mixed in."
        )
    ...
    if not args.smoke:
        for fname, text in paragraphs.items():
            with open(
                os.path.join(CORPUS_DIR, f"{fname}.txt"), "w", encoding="utf-8"
            ) as f:
                f.write(text)
    ...
    print(
        f"  unique paragraphs     : {len(paragraphs):,}"
    ...
    if n_bad_gold:
        print(
            f"  questions dropped     : {n_bad_gold} "
    ...
    for t in ("bridge", "comparison"):
        ...
        role = (
            "KG's home turf — needs a bridge entity"
            if t == "bridge"
    ...
    print(
        f"  questions -> questions_hotpotqa_bridge.json, "
    ...
    print(
        f"{DIM}    • triples_cache.json is PubLayNet's — this corpus needs "
        f"its own{RESET}"
    )
