        ...
        messages=[
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": "Describe this region from a scientific document in one concise sentence. "
...
def ingest(limit: int, smoke: bool = False) -> None:
    ...
    def flush():
        ...
        if current_key is None or not current_tokens:
            return
        n_t, n_i = process_page(
            current_key, current_img_bytes, current_tokens, captions, smoke=smoke
        )
        n_text_total += n_t
        n_images_total += n_i
