...
QSETS = [
    os.path.join("data", "questions", "questions_publaynet_text.json"),
    os.path.join("data", "questions", "questions_publaynet_multihop.json"),
    os.path.join("data", "questions", "questions_publaynet_figures.json"),
]
...
def main():
    ...
    ap.add_argument("--only", nargs="+", default=None, help="only these models")
    ap.add_argument("--skip", nargs="+", default=[], help="skip these models")
    args = ap.parse_args()
    ...
    for i, (model, qfile) in enumerate(plan, 1):
        ...
        cmd = [
            ...
            "-u",
            "-m",
            "multimodal_graph_rag.pipelines.evaluate",
            qfile,
            "--model",
            model,
        ]
    ...
    print("=" * 72)
    print(
        f"MATRIX COMPLETE — {len(plan) - len(failures)}/{len(plan)} succeeded, "
