...
def ingest(limit: int, smoke: bool = False, category_filter: str | None = None) -> None:
    ...
    dataset = load_dataset(
        HF_DATASET, split="train", streaming=True, trust_remote_code=False
    )
    ...
    for row in dataset:
        ...
        for cat_id, bbox in zip(cat_ids, bboxes):
            ...
            # ── Save image crops ──────────────────────────────────────────────
            save_as_image = (category in IMAGE_CATEGORIES) or (
                label in ALSO_SAVE_AS_IMAGE
            )
            if save_as_image:
                image_name = (
                    f"{page_hash}_{label.lower().replace('-', '_')}{figure_count}.png"
                )
