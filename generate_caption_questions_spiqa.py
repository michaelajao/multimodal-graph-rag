...
from generate_questions import (
    claude_client, parse_json_array, leaks_modality, has_orphan_reference,
    CLAUDE_MODEL, save, load_existing,
)

load_dotenv()

IMAGE_DIR  = "spiqa_images"
PIXEL_FILE = "questions_spiqa_figures.json"
OUT_FILE   = "questions_spiqa_figures_caption.json"
...
def main(pixel_file):
    ...
    print(f"Generating caption-answerable questions for the SAME {len(crops)} "
    ...
    for name in crops:
        ...
        if qa is None:
            ...
            continue
        out.append({"q": qa["q"], "source": name,
                    "answer": qa["answer"], "type": "figure"})
    ...
    def diag(items, label):
        n = len(items)
        in_cap = sum(answer_in_caption(i["answer"], captions.get(i["source"], ""))
                     for i in items)
        mean_len = sum(len(i["answer"]) for i in items) / n if n else 0
        print(f"  {label:<22} n={n:<3} answers-in-caption={in_cap}/{n} "
    ...
    if pix_in_cap:
        print(f"  WARNING: {pix_in_cap} pixel-only answers appear in their captions. "
