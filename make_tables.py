...
Selection rule: for each (question_set, model, system), the NEWEST run whose
baseline text retrieval is non-zero (guards against wrong-corpus runs, which
show Recall 0.000 on .txt-gold sets). Older runs contribute only systems the
newest run lacks (e.g. +multimodal/+both on the cross set come from the
four-way runs; baseline/+KG/+KGret from the newer three-way runs).
...
CONTAMINATED_SPIQA_FIGS = {
    "1809.00263v5-Figure12-1.png", "1906.10843v1-Figure5-1.png",
    "1705.02946v3-Figure6-1.png",  "1803.01128v3-Table2-1.png",
    "1802.07351v2-Figure10-1.png",
}

GRAPH_SETS = ["publaynet_multihop", "spiqa_multihop", "spiqa_multihop_cross",
              "hotpotqa_bridge", "hotpotqa_comparison"]
SET_LABEL = {
...
def select(runs):
    ...
    newest valid run per system, older runs fill missing systems."""
    cell = {}
...
def main():
    ...
    ap.add_argument("--spiqa-figq", default="questions_spiqa_figures.json")
    args = ap.parse_args()
    ...
    write_table(args.out, "T1_graph_three_way", header, rows,
    ...
    header = ["set", "system", "AllGoldFound", "mean acc", "mean faith",
              "Δacc vs baseline"]
    rows = []
    for qset in ["hotpotqa_bridge", "hotpotqa_comparison"]:
        ...
        for system in ["baseline", "+KG", "+KGret"]:
            ...
            if system == "baseline":
                base_acc = macc
            rows.append([SET_LABEL[qset], system, comp, fmt(macc),
                        fmt(sum(faiths) / len(faiths)),
                        f"{macc - base_acc:+.3f}" if base_acc is not None else "—"])
    write_table(args.out, "T2_bridge_vs_comparison", header, rows,
    ...
    header = ["question set", "system", "acc | complete", "acc | incomplete",
              "n complete", "n incomplete"]
    rows = []
    for qset in GRAPH_SETS:
        for system in ["baseline", "+KG", "+KGret"]:
            c1 = c0 = a1 = a0 = 0
            for model in MODELS:
                ...
                for r in detail_rows(c[1]):
                    ...
                    if comp:
                        ...
                        c0 += 1; a0 += acc
            if c1 + c0:
                rows.append([SET_LABEL[qset], system,
                             fmt(a1 / c1) if c1 else "—",
                             fmt(a0 / c0) if c0 else "—",
                             c1, c0])
    write_table(args.out, "T3_evidence_conditioned", header, rows,
    ...
    if os.path.exists(args.spiqa_figq):
        ...
        for q in figq:
            ...
            if src in CONTAMINATED_SPIQA_FIGS:
                ...
                paper = os.path.join(args.spiqa_corpus,
                                     src.split("-")[0] + ".txt")
                intext = False
                if os.path.exists(paper):
                    with open(paper, encoding="utf-8") as pf:
                        intext = (len(norm(q["answer"])) >= 2
                                  and norm(q["answer"]) in norm(pf.read()))
                tiers[src] = "text-recoverable" if intext else "strict-pixel"
            else:
                tiers[src] = "clean (corpus absent: tier unknown)"
        if not corpus_ok:
            print(f"  NOTE: {args.spiqa_corpus}/ not found — SPIQA figure "
                  "tiering limited to the caption exclusion.")

    header = ["set / tier", "n", "model", "text-only baseline acc",
              "+multimodal acc"]
    rows = []
    for qset in ["publaynet_figures", "publaynet_figures_caption"]:
        for model in MODELS:
            ...
            if cb:
                rows.append([qset, cb[0]["n_questions"], model,
                             fmt(cb[0]["acc"]),
    ...
    for model in MODELS:
        ...
        for tier in tier_names:
            ...
            mm = (sum(int(float(det[s]["+multimodal_acc"])) for s in srcs)
                  if cm and "+multimodal_acc" in next(iter(det.values())) else None)
            rows.append([f"spiqa_figures [{tier}]", len(srcs), model,
                         f"{b}/{len(srcs)}",
                         f"{mm}/{len(srcs)}" if mm is not None else "—"])
    write_table(args.out, "T4_figure_integrity", header, rows,
    ...
    header = ["question set", "model", "system", "recall", "complete", "acc",
              "faith", "rel"]
    rows = []
    for (qset, model, system), (r, _, _) in sorted(cell.items()):
        if model in DIAG:
            continue
        rows.append([qset, model, system, r["recall"], r["complete"],
                     r["acc"], r["faith"], r["rel"]])
    write_table(args.out, "T5_all_runs_flat", header, rows,
                "Every selected (newest valid) cell, flat — the source of "
                "truth behind T1–T4 and the per-corpus four-way tables.")

    # ── T6: efficiency ──────────────────────────────────────────────────────
    header = ["question set", "model", "system", "in_tok/q", "out_tok/q",
              "latency ms", "cost/100q $"]
    rows = []
    for qset in ["publaynet_figures", "spiqa_multihop_cross",
                 "hotpotqa_bridge"]:
        for model in MODELS:
            for system in SYSTEMS:
                ...
                if c:
                    r = c[0]
                    rows.append([qset, model, system, r["mean_in_tok"],
                                 r["mean_out_tok"], r["mean_latency_ms"],
                                 r["cost_per_100q_usd"]])
